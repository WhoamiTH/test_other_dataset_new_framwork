...
def transform_data_to_test_form_data_bm_and_im(test_data, reference_data):
    temd = []
    length_test_data = test_data.shape[0]
    length_reference_data = reference_data.shape[0]
    test_time = length_reference_data

    test_index = [i for i in range(length_test_data)]
    reference_index = [i for i in range(length_reference_data)]

    test_index = np.repeat(test_index, test_time)
    reference_index = np.tile(reference_index, length_test_data)
...
def transform_data_to_test_form_data_both(test_data, border_majority, informative_minority):
    temd = []

    reference_data = np.vstack((border_majority, informative_minority))
...
#     # print(Data_nega)
#     # print(length_posi)
#     # print(tem_data_nega_list)

#     # print(length_posi)
#     # print(len(tem_data_nega_list))
...
def generate_transformed_data_index(headindex, tailindex):
    data_headindex = np.tile(headindex, len(tailindex))
    data_tailindex = np.repeat(tailindex, len(headindex))
    return data_headindex, data_tailindex

def generate_batch_data(positive_index, negative_index, positive_length, negative_length, batch_size):
    ...
    # print(times)
    if positive_length<=batch_size:
        current_batch_size = positive_length
    else:
        current_batch_size = batch_size

    positive_data_index = np.random.choice(positive_index, current_batch_size, replace=False)
    negative_data_index = np.random.choice(negative_index, times*current_batch_size, replace=False)
    select_negative_data_index = np.random.choice(negative_index, current_batch_size, replace=False)
    return positive_data_index, negative_data_index, select_negative_data_index



def generate_batch_data_bm(positive_index, negative_index, border_majority_index, positive_length, negative_length, border_majority_length, batch_size):
    times = negative_length / positive_length
    # numpy.transpose([numpy.tile(x, len(y)), numpy.repeat(y, len(x))])
    times = int(times)
    ...
    # print(times)
    if border_majority_length<=batch_size:
        current_batch_size = border_majority_length
    else:
        current_batch_size = batch_size

    positive_data_index = np.random.choice(positive_index, current_batch_size, replace=False)
    negative_data_index = np.random.choice(negative_index, times*current_batch_size, replace=False)
    select_negative_data_index = np.random.choice(border_majority_index, current_batch_size, replace=False)
    return positive_data_index, negative_data_index, select_negative_data_index


def generate_batch_data_im(positive_index, negative_index, informative_minority_index, positive_length, negative_length, informative_minority_length, batch_size):
    times = negative_length / positive_length
    # numpy.transpose([numpy.tile(x, len(y)), numpy.repeat(y, len(x))])
    times = int(times)
    ...
    # print(times)
    if informative_minority_length<=batch_size:
        current_batch_size = informative_minority_length
    else:
        current_batch_size = batch_size

    positive_data_index = np.random.choice(informative_minority_index, current_batch_size, replace=False)
    negative_data_index = np.random.choice(negative_index, times*current_batch_size, replace=False)
    select_negative_data_index = np.random.choice(negative_index, current_batch_size, replace=False)
    return positive_data_index, negative_data_index, select_negative_data_index

def generate_batch_data_both(positive_index, negative_index, border_majority_index, informative_minority_index, positive_length, negative_length, border_majority_length, informative_minority_length, batch_size):
    times = negative_length / positive_length
    # numpy.transpose([numpy.tile(x, len(y)), numpy.repeat(y, len(x))])
    times = int(times)
...
# def generate_transformed_label(length, label):
#     label_zero = np.zeros((length,1), dtype=int)
#     # label_zero = label_zero.reshape(-1,1)
#     label_ones = np.ones((length,1), dtype=int)
#     # label_ones = label_ones.reshape(-1,1)

#     if label > 0:
#         transformed_label = np.hstack((label_ones, label_zero))
#     else:
#         transformed_label = np.hstack((label_zero, label_ones))
...
def generate_transformed_label(length, label):
    if label > 0:
        transformed_label = np.ones((length,1), dtype=int)
        # transformed_label = label_ones.reshape(-1,1)
    else:
        transformed_label = np.zeros((length,1), dtype=int)
        # transformed_label = label_zero.reshape(-1,1)
    return transformed_label
