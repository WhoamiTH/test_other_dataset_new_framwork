...
for record_number in range(1, total_record_number+1):
    record_name = 'execute_first_head_half_{0}.sh'.format(record_number)
...
for record_number in range(1, total_record_number+1):
    record_name = 'execute_first_tail_half_{0}.sh'.format(record_number)
    file = open(record_name,'w')
...
for record_number in range(1, total_record_number+1):
    record_name = 'execute_second_head_half_{0}.sh'.format(record_number)
    file = open(record_name,'w')
...
for record_number in range(1, total_record_number+1):
    record_name = 'execute_second_tail_half_{0}.sh'.format(record_number)
    file = open(record_name,'w')
