...
first_hidden1 = tf.layers.dense(inputs=x, units=4*single_input_size, use_bias=True, activation=tf.nn.relu, name='hidden1')
y_pred = tf.layers.dense(inputs=first_hidden1, units=num_class, activation=tf.nn.sigmoid, name='y_pred')


second_hidden1 = tf.layers.dense(inputs=x_2, units=4*single_input_size, use_bias=True, activation=tf.nn.relu, name='hidden1', reuse=True)
