...
def next_batch(train_data, train_label, batch_size=2):
    x_examples      = []
    y_examples      = []
    ...
    x_examples.append(train_data[batch_index])
    y_examples.append(train_label[batch_index])
...
def repeat_data(positive_data, negative_data):
    positive_length = positive_data.shape[0]
    negative_length = negative_data.shape[0]

    ir = math.floor(negative_length/positive_length)

    new_positive_data = positive_data
    for i in range(1, ir):
        new_positive_data = np.vstack((new_positive_data, positive_data))
    new_positive_length = new_positive_data.shape[0]

    negative_label = np.zeros(negative_length).reshape((-1,1))
    positive_label = np.ones(new_positive_length).reshape((-1,1))

    new_negative_data = np.hstack((negative_data, negative_label))
    new_positive_data = np.hstack((new_positive_data, positive_label))

    new_data = np.vstack((new_negative_data, new_positive_data))
