...
for record_number in range(1, total_record_number+1):
    ...
    for i in range(1, 1+test_number):
        ...
        model_name
        """
        file.write('python train.py file_name=../1_year_data/{7}_train_{8}.csv model_record_path=../1_year_result/model_{1}/ file_record_path=../1_year_result/record_{2}/ method_name={3} scaler_name=scaler_{4}.m pca=False pca_name=pca_{5}.m model_name=model_{6}/my_model\n'.format(i, record_number, record_number, method_name, i, i, i, file_name_pre, i))
        """
        test parameter list:
        file_name
        model_record_path
        ...
        record_name
        """
        file.write('python test.py train_file_name=../1_year_data/{8}_train_{0}.csv test_file_name=../1_year_data/{8}_test_{7}.csv model_record_path=../1_year_result/model_{1}/ file_record_path=../1_year_result/record_{2}/ method_name={3} scaler_name=scaler_{4}.m model_name=model_{6} record_name=test_result_{7}.csv\n'.format(i, record_number, record_number, method_name, i, i, i, i, file_name_pre))
        """
